...
$(document).ready(function() {
        ...
        for (var i = 0; i <= 3; i++) {
            (function (i) {
                setTimeout(() => {
                    switch (x) {
                        case 0: 
                            $('#ready-text').text('En sus marcas'); 
                            break
                        case 1: 
                            $('#ready-text').text('Listos'); 
                            break
                        case 2: 
                            $('#ready-text').text('Empiecen!'); 
                            break
                        case 3: 
                            startGame();
                            break
    ...
    function loadQuestion(questionIndex) {
        $('#question-number').text('Pregunta ' + (questionIndex + 1));
    ...
    function showScore() {
        ...
        $('#correct').text(correctAnswers.toString() + ' Correcto');
        $('#incorrect').text(incorrectAnswers.toString() + ' Incorrecto'); 
        $('.image').attr('id', '1').attr('src', 'images/1.png'); 
        $('#score').show(1000);
        $('#done').click(function(){
            reset();
        });       
    ...
    function checkAnswer(questionIndex, answerIndex) {
        ...
        if (answerIndex === answer) {
            ...
            $('#timer').css('color', 'green').text('Correcto');
            setTimeout(() => {
                // Resets timer text
                $('#timer').text('60');
                // Loads next question
            ...
            $('#timer').css('color', 'red').text('Incorrecto').shake({
                interval: 100,
                distance: 20,
                times: 5
            });
    ...
    function updateImage(imageId) {
        switch (imageId) {
            case '1': 
                $('.image').attr('id', '1').attr('src', 'images/a.png'); 
                break
            case '2': 
                $('.image').attr('id', '2').attr('src', 'images/b.png'); 
                break
            case '3': 
                $('.image').attr('id', '3').attr('src', 'images/c.png');
                break
            case '4': 
                $('.image').attr('id', '4').attr('src', 'images/d.png');
                break
            case '5': 
                $('.image').attr('id', '5').attr('src', 'images/e.png'); 
                break
